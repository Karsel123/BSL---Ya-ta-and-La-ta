...
st.markdown(
    "<div style='background-color: #f2f2f2; padding: 20px; text-align: center; border-radius: 10px;'><h1 style='color: black;'>BSL - La ta and Ya ta</h1></div>",
...
# Create a function to take a screenshot and save it to the desktop
def take_screenshot():
    desktop_path = os.path.expanduser("D:\screenshot")
    save_path = os.path.join(desktop_path, "screenshot.png")
...
while running:
    ...
    if results.multi_hand_landmarks is not None:  # Check if hand landmarks are detected
    ...
    if results.multi_hand_landmarks is None or len(results.multi_hand_landmarks) != 2:  # If no hand landmarks or only one hand landmark is detected
        white_background = np.ones((frame.shape[0], frame.shape[1], 3), dtype=np.uint8) * 255
